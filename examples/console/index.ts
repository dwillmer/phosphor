...
class ConsoleView extends Widget {
  ...
  constructor() {
    ...
    var text2 = "Another test";
    this._ctx.fillText( text2, 10, this._vpos );
    //var metrics2 = this._ctx.measureText( text2 );
    this._vpos += this._getTextHeight( text2 );*/
    ...
    this._ctx.drawImage( image, 10, this._vpos );
    var implace = this._vpos;
    image.onload = () => {
      this._ctx.drawImage( image, 10, implace );
    }
    ...
    /*var text3 = "In[5]> ";
    this._ctx.fillText( text3, 10, this._vpos );
    this._vpos += this._getTextHeight( text3 );*/

    this._row_string = "";
    window.addEventListener(
      "keypress",
      (evt: Event) => { this._keyPressHandler(evt, this._ctx) },
      true
    );

    window.addEventListener(
      "keydown",
      (evt: Event) => { this._onKeyDownHandler(evt, this._ctx) },
      true
    );
  ...
  private _onKeyDownHandler( event: any, ctx: any ): string {
  ...
  private _keyPressHandler(event: any, ctx: any): string {
    var key = event.which || event.keyCode;
